...
A manager can replace any player at any time. The scorecard needs to capture exactly when and how each substitution happened.

All substitution indicators use the same visual pattern: **small dotted squares** (5px squares with 4px gaps). The position of the line tells you the type of sub and when it happened relative to the at-bat.
...
A pinch runner replaces a batter who has already reached base. The original batter goes to the dugout and a faster runner takes their place on the bases.

**Visual**: Dotted line on the **RIGHT side** of the play cell.

The line is on the right because it represents the "exit door." The play in the cell is what the original batter did. The pinch runner takes over from there.
...
A defensive switch event can never overwrite a PH or PR substitution type. This prevents a pinch hitter's line from disappearing when they take the field in the next half-inning.
